async function renderCenters(){
    ...
    let CDhtml = '';
        for (let i=0; i<CenterDisplayy.length; i++){
            ...
            <button 
            onclick = "newpin(CenterDisplayy[${i}])"
...
/* Doesn't work, need to find how to make it delete at given index and also code the app.js for /deleteCenter
const DeleteButton = document.getElementById("DeleteButton"); 
DeleteButton.addEventListener ('click', async function (){
    try {
        const response = await fetch(`/deleteCenter/${index}`, {
            method: 'DELETE',
            headers: {
